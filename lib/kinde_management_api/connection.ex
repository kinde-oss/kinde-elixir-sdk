...
defmodule KindeManagementAPI.Connection do
  @moduledoc """
  Handle Tesla connections for KindeManagementAPI.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, KindeManagementAPI.Connection,
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:kinde_management_api, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - KindeManagementAPI 1 - elixir"
        )
      )
